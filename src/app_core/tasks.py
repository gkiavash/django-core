...
from celery import Task as CeleryTask, current_app
...
from app_core import models, utils, mixins
from app_core.exceptions import RuntimeStateException
...
class StateTransitionTask(Task):
    ...
    """
    name = 'app_core.StateTransitionTask'
...
class RuntimeStateChangeTask(Task):
    ...
    """
    name = 'app_core.RuntimeStateChangeTask'
...
class BackendMethodTask(RuntimeStateChangeTask, StateTransitionTask):
    ...
    name = 'app_core.BackendMethodTask'
    ...
    def execute(self, instance, backend_method, *args, **kwargs):
        ...
        try:
            instance = getattr(backend, backend_method)(instance, *args, **kwargs)
...
class IndependentBackendMethodTask(BackendMethodTask):
    ...
    name = 'app_core.IndependentBackendMethodTask'
...
class ErrorMessageTask(Task):
    ...
    as input argument.
    """
    name = 'app_core.ErrorMessageTask'
    ...
    def save_error_message(self, instance):
        if isinstance(instance, mixins.ErrorMessageMixin):
            ...
            if instance.state != mixins.StateMixin.States.ERRED:
...
class ErrorStateTransitionTask(ErrorMessageTask, StateTransitionTask):
    ...
    as input argument.
    """
    name = 'app_core.ErrorStateTransitionTask'
...
class PreApplyExecutorTask(Task):
    ...
    name = 'app_core.PreApplyExecutorTask'
...
class PollRuntimeStateTask(Task):
    ...
    name = 'app_core.PollRuntimeStateTask'
    ...
    def execute(
     ...
     #   getattr(backend, kwargs['backend_pull_method'])(instance)
        instance.refresh_from_db()
        if instance.runtime_state not in (success_state, erred_state, deleted_state):
        #if instance.runtime_state not in (kwargs['success_state'], kwargs['erred_state'],kwargs['deleted_state']):
            self.retry()
        elif instance.runtime_state == erred_state:
        #elif instance.runtime_state == kwargs['erred_state']:
            raise RuntimeStateException(
                '%s (PK: %s) runtime state become erred: %s'
                % (instance.__class__.__name__, instance.pk, erred_state)
                #% (instance.__class__.__name__, instance.pk, kwargs['erred_state'])
            )
        return instance
...
class PollStateTask(Task):
    max_retries = 1200
    default_retry_delay = 5
    name = 'app_core.PollStateTask'

    def execute(self, instance, *args, **kwargs):
        if instance.state not in (
            mixins.StateMixin.States.OK,
            mixins.StateMixin.States.ERRED,
        ):
            self.retry()
...
class PollBackendCheckTask(Task):
    max_retries = 600
    default_retry_delay = 5
    name = 'app_core.PollBackendCheckTask'
...
current_app.tasks.register(Task())
current_app.tasks.register(EmptyTask())
current_app.tasks.register(StateTransitionTask())
current_app.tasks.register(RuntimeStateChangeTask())
current_app.tasks.register(BackendMethodTask())
current_app.tasks.register(IndependentBackendMethodTask())
current_app.tasks.register(DeletionTask())
current_app.tasks.register(ErrorMessageTask())
current_app.tasks.register(ErrorStateTransitionTask())
current_app.tasks.register(PreApplyExecutorTask())
current_app.tasks.register(BackgroundTask())
current_app.tasks.register(PollRuntimeStateTask())
current_app.tasks.register(PollStateTask())
current_app.tasks.register(PollBackendCheckTask())
